...
from aws_maintenance_window_reporter.maintenance_window import get_next_maintenance_window
from collections import namedtuple

from aws_maintenance_window_reporter.environment_parameter import get as get_parameter

MaintenanceAction = namedtuple('MaintenanceAction', ['instance_id', 'instance_type', 'service', 'not_before', 'description'])


def create_maintenance_action_from_rds_arn(arn: str, not_before: Optional[datetime], description: Optional[str]) -> MaintenanceAction:
    ...
    return MaintenanceAction(resource_id, rds_resource_type_to_datadog_tag.get(resource_type, resource_type), 'rds', not_before, description)
    ...
    for response in client.get_paginator('describe_db_instances').paginate():
        for instance in response['DBInstances']:
            if instance.get('PendingModifiedValues'):
                ...
                result.append(create_maintenance_action_from_rds_arn(arn, not_before, "Pending modified values"))
    ...
    for response in client.get_paginator('describe_pending_maintenance_actions').paginate():
        for action in response['PendingMaintenanceActions']:
            ...
            for detail in action['PendingMaintenanceActionDetails']:
                ...
                result.append(create_maintenance_action_from_rds_arn(action['ResourceIdentifier'], not_before, description))
        ...
        for instance_status in response['InstanceStatuses']:
            for event in instance_status['Events']:
                ...
                if description and not description.startswith('[Completed]'):
                    result.append(MaintenanceAction(instance_status['InstanceId'], 'instance-id', 'ec2', not_before, description))
    return result

def send_metric(action: MaintenanceAction, timestamp: datetime, do_send_metrics: bool):
    log.info("%s %s %s: %s on %s", action.service, action.instance_type, action.instance_id, action.description, action.not_before, )
    if do_send_metrics:
